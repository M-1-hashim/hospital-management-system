  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [medRes, rxRes, patRes] = await Promise.allSettled([
        ...
        fetch('/api/pharmacy/prescriptions').then(r => r.json()),
    ...
    <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.06 } } }} className="space-y-6">
      ...
      {(lowStockCount > 0 || expiringCount > 0) && (
        <motion.div variants={fadeUp} className="flex flex-wrap gap-3">
          {lowStockCount > 0 && <Card className="border-amber-300 dark:border-amber-700"><CardContent className="p-3 flex items-center gap-3"><AlertTriangle className="size-5 text-amber-600" /><div><p className="font-medium text-sm text-amber-700 dark:text-amber-400">{lowStockCount} {isRTL ? 'داروی کمبود موجودی' : 'Low Stock Medicines'}</p></div></CardContent></Card>}
          {expiringCount > 0 && <Card className="border-red-300 dark:border-red-700"><CardContent className="p-3 flex items-center gap-3"><AlertCircle className="size-5 text-red-600" /><div><p className="font-medium text-sm text-red-700 dark:text-red-400">{expiringCount} {isRTL ? 'داروی نزدیک انقضا' : 'Expiring Medicines'}</p></div></CardContent></Card>}
        </motion.div>
      )}

      <Tabs defaultValue="inventory">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <TabsList><TabsTrigger value="inventory"><Package className="size-4" />{t('pharmacy')}</TabsTrigger><TabsTrigger value="prescriptions"><FileText className="size-4" />{isRTL ? 'نسخه‌ها' : 'Prescriptions'}</TabsTrigger></TabsList>
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => { setRxDialogOpen(true); }}><FileText className="size-4" />{isRTL ? 'نسخه جدید' : 'New Rx'}</Button>
        ...
        <TabsContent value="inventory" className="space-y-4 mt-4">
          <motion.div variants={fadeUp} className="flex flex-wrap gap-3">
            ...
            <Button variant={lowOnly ? 'default' : 'outline'} size="sm" onClick={() => setLowOnly(!lowOnly)} className={lowOnly ? 'bg-amber-600' : ''}><AlertTriangle className="size-4" />{isRTL ? 'کمبود' : 'Low Stock'}</Button>
            <Button variant={expiringOnly ? 'default' : 'outline'} size="sm" onClick={() => setExpiringOnly(!expiringOnly)} className={expiringOnly ? 'bg-red-600' : ''}><CalendarDays className="size-4" />{isRTL ? 'انقضا' : 'Expiring'}</Button>
          </motion.div>

          <Card>
            <CardContent className="p-0 overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b bg-muted/50"><th className="p-3 text-start font-medium">{t('name')}</th><th className="p-3 text-start font-medium">{isRTL ? 'دسته' : 'Category'}</th><th className="p-3 text-start font-medium">{isRTL ? 'قیمت' : 'Price'}</th><th className="p-3 text-start font-medium">{isRTL ? 'موجودی' : 'Stock'}</th><th className="p-3 text-start font-medium">{isRTL ? 'حداقل' : 'Min'}</th><th className="p-3 text-start font-medium">{isRTL ? 'انقضا' : 'Expiry'}</th><th className="p-3 text-start font-medium">{t('actions')}</th></tr></thead>
                <tbody>
                  {medicines.map((med) => (
                    <tr key={med.id} className="border-b hover:bg-muted/30">
                      ...
                      <td className="p-3"><div className="flex gap-1">
                        ...
                        <Button size="icon" variant="ghost" className="size-7 text-destructive" onClick={() => handleDeleteMedicine(med.id)}><Trash2 className="size-3.5" /></Button>
                      </div></td>
                    </tr>
                  ))}
                </tbody>
      ...
      <Dialog open={dialogOpen} onOpenChange={v => { setDialogOpen(v); if (!v) setSelectedMed(null); }}>
        <DialogContent className="max-w-lg"><DialogHeader><DialogTitle>{selectedMed ? t('edit') : t('add')} {isRTL ? 'دارو' : 'Medicine'}</DialogTitle></DialogHeader>
          <div className="grid gap-3 py-2">
            ...
            <div className="grid grid-cols-2 gap-3"><div><Label>{isRTL ? 'دسته‌بندی' : 'Category'}</Label><Input value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} /></div><div><Label>{isRTL ? 'شرکت سازنده' : 'Manufacturer'}</Label><Input value={form.manufacturer} onChange={e => setForm({ ...form, manufacturer: e.target.value })} /></div></div>
            <div className="grid grid-cols-3 gap-3"><div><Label>{isRTL ? 'شکل دارو' : 'Form'}</Label><Select value={form.dosageForm} onValueChange={v => setForm({ ...form, dosageForm: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{['tablet','capsule','syrup','injection','cream','drops'].map(f => <SelectItem key={f} value={f}>{f}</SelectItem>)}</SelectContent></Select></div><div><Label>{isRTL ? 'قدرت' : 'Strength'}</Label><Input value={form.strength} onChange={e => setForm({ ...form, strength: e.target.value })} placeholder="500mg" /></div><div><Label>{isRTL ? 'شماره بچ' : 'Batch#'}</Label><Input value={form.batchNumber} onChange={e => setForm({ ...form, batchNumber: e.target.value })} /></div></div>
            <div className="grid grid-cols-3 gap-3"><div><Label>{isRTL ? 'قیمت' : 'Price'}</Label><Input type="number" value={form.price} onChange={e => setForm({ ...form, price: e.target.value })} /></div><div><Label>{isRTL ? 'موجودی' : 'Stock'}</Label><Input type="number" value={form.stock} onChange={e => setForm({ ...form, stock: e.target.value })} /></div><div><Label>{isRTL ? 'حداقل موجودی' : 'Min Stock'}</Label><Input type="number" value={form.minStock} onChange={e => setForm({ ...form, minStock: e.target.value })} /></div></div>
            <div><Label>{isRTL ? 'تاریخ انقضا' : 'Expiry Date'}</Label><Input type="date" value={form.expiryDate} onChange={e => setForm({ ...form, expiryDate: e.target.value })} /></div>
      ...
      <Dialog open={rxDialogOpen} onOpenChange={setRxDialogOpen}>
        <DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>{isRTL ? 'نسخه جدید' : 'New Prescription'}</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-3">
              <div><Label>{t('patient')}</Label><Select value={rxForm.patientId} onValueChange={v => setRxForm({ ...rxForm, patientId: v })}><SelectTrigger><SelectValue placeholder={isRTL ? 'انتخاب بیمار' : 'Select patient'} /></SelectTrigger><SelectContent>{patients.map(p => <SelectItem key={p.id} value={p.id}>{p.firstName} {p.lastName}</SelectItem>)}</SelectContent></Select></div>
              <div><Label>{t('notes')}</Label><Input value={rxForm.notes} onChange={e => setRxForm({ ...rxForm, notes: e.target.value })} /></div>
            </div>
            <div className="space-y-2">
              <div className="flex justify-between items-center"><Label>{isRTL ? 'داروها' : 'Medicines'}</Label><Button size="sm" variant="outline" onClick={addRxItem}><Plus className="size-3" />{t('add')}</Button></div>
              {rxItems.map((item, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 items-end">
                  <div className="col-span-3"><Label className="text-xs">{isRTL ? 'دارو' : 'Medicine'}</Label><Select value={item.medicineId} onValueChange={v => { updateRxItem(i, 'medicineId', v); const med = medicines.find(m => m.id === v); if (med) updateRxItem(i, 'unitPrice', med.price); }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{medicines.filter(m => m.isActive).map(m => <SelectItem key={m.id} value={m.id}>{m.name}{m.nameFa ? ` (${m.nameFa})` : ''}</SelectItem>)}</SelectContent></Select></div>
                  <div className="col-span-2"><Label className="text-xs">{isRTL ? 'دوز' : 'Dosage'}</Label><Input value={item.dosage} onChange={e => updateRxItem(i, 'dosage', e.target.value)} placeholder="1 tab" /></div>
                  <div className="col-span-2"><Label className="text-xs">{isRTL ? 'تناوب' : 'Frequency'}</Label><Select value={item.frequency} onValueChange={v => updateRxItem(i, 'frequency', v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{['daily','twice','three_times','four_times'].map(f => <SelectItem key={f} value={f}>{f}</SelectItem>)}</SelectContent></Select></div>
                  <div className="col-span-2"><Label className="text-xs">{isRTL ? 'مدت' : 'Duration'}</Label><Select value={item.duration} onValueChange={v => updateRxItem(i, 'duration', v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{['3_days','1_week','2_weeks','1_month'].map(d => <SelectItem key={d} value={d}>{d}</SelectItem>)}</SelectContent></Select></div>
                  <div className="col-span-2"><Label className="text-xs">{isRTL ? 'تعداد' : 'Qty'}</Label><Input type="number" value={item.quantity} onChange={e => updateRxItem(i, 'quantity', Number(e.target.value))} /></div>
            ...
            <Button className="bg-emerald-600 hover:bg-emerald-700" onClick={handleSaveRx}>{isRTL ? 'ثبت نسخه' : 'Save Prescription'}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </motion.div>
